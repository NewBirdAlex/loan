<template>
	<div id="wrap">
		...
		<div class="bgWhite">
			<ul>
				<!--遍历state中的upload项-->
				<router-link :to="combineRouter(uploadItem)" tag="li" :class="{active: isUpload(uploadItem)}" v-for="(uploadItem, index) in uploadList" :key="uploadItem">{{uploadItem}}
					<i></i>
				</router-link>
			</ul>
		</div>
		...
		<input class="nextStep" type="button" value="提交" :disabled="!checkboxValid" @click='pop=!pop'>
		<div class="pop" v-show="pop">
			<div class="content">
				<img src="../../assets/images/delete.png" @click.stop="pop=!pop" class="close" />
				<p>
...
<script>
	...
	export default {
		...
		mounted() {
			this.$store.commit('SAVE_TEMP_ROUTER', {
				data: this.nonceRouter
			})
		},
		computed: {
			...mapGetters([
			...
			jointParam() {
				return {
					name: 'commonApply',
					params: Object.assign({}, this.nonceRouter, {
						group: '按揭共同申请人信息'
					})
				}
			},
		...
		methods: {
			...
			submit() {
				//不同的产品名称提交
				if(this.productName=="中行爱家"){
					var productData = {"中行爱家详细信息": this.parseJSON()}
				}else{
					var productData = {"按揭详细信息": this.parseJSON()}
				}
...
<style scoped lang="less">
	...
	.check {
		...
		input[type=checkbox]:checked+i {
			background: url(../../assets/images/pick.png);
			background-size: 100% 100%;
		}
		input[type=checkbox]+i {
			background: url(../../assets/images/unpick.png);
			background-size: 100% 100%;
		}
	}
