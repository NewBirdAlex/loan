...
let originFormData = JSON.parse(JSON.stringify(formData));
...
for(let value in formData) {
	if(/step/.test(value)) {
		formData[value].map((ele, ind) => {
			...
			formData[value][ind] = Object.assign({}, ele, {
				value: vary
			})
		})
	}
...
let mutations = {
	...
	SET_FORM(state){
		state.formData = JSON.parse(JSON.stringify(state.originFormData));
	},
	SET_PRODUCTNAME(state,obj){
	...
	ADD_PROPETY(state) {
		...
		arr.map(function(item){
			state.formData.step1.push(item)
		});
