...
let mutations = {
	...
	MODIFY_DATUM(state, {
		...
		state.activeDatum = { ...state['activeDatum'],
			...data
		}
	},
	...
	UPDATE_PRODUCTID(state, {
		data
	}) {
		state.productId = data
	},
	UPDATE_MODIFY_ORDER_PARAMS(state, {
		data
	}) {
		state.modifyOrderParams = { ...state.modifyOrderParams,
			...data
		}
	},
...
import modifyOrder from '../../api/modifyOrder'
let actions = {
	fetchModifyOrder({
		commit,
		state
